...
Запуск: python -m modules.migrations.python.migrate_to_postgres
"""
...
def create_postgres_tables():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS product_results (
            ...
            match_level VARCHAR(50) DEFAULT 'partial',
            rank_score REAL DEFAULT 0,
            tags TEXT,
            related_terms TEXT,
            search_tokens TEXT,
            source_level VARCHAR(50) DEFAULT 'partial'
        );
    """)
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_product_results_rank ON product_results(rank_score DESC);")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_product_results_match_level ON product_results(match_level);")
    ...
    # РўР°Р±Р»РёС†Р° С‚СЂРµРЅРґРѕРІ YouTube
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS youtube_results (
            ...
            thumbnail_quality VARCHAR(50),
            clickbait_probability VARCHAR(50),
            emotion_score REAL
        );
    """)
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_youtube_video_id ON youtube_results(video_id);")
...
def verify_migration():
    ...
    tables = ['users', 'sites', 'examples', 'product_results', 'social_results', 'youtube_results']
    sqlite_tables = set(get_sqlite_tables())
    
    for table in tables:
        if table not in sqlite_tables:
            print(f"   ⏭ {table}: нет в SQLite, пропускаем")
            continue
        try:
