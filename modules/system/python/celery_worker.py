...
celery_app = Celery(
    ...
    broker=REDIS_URL,
    backend=REDIS_URL,
    include=['modules.system.python.celery_worker']
)
...
celery_app.conf.beat_schedule = {
    ...
    'cleanup-online-users': {
        'task': 'modules.system.python.celery_worker.cleanup_online_users',
        'schedule': crontab(minute=0),  # Каждый час
    },
    # Синхронизация с главным сайтом каждые 10 минут
    'sync-main-site': {
        'task': 'modules.system.python.celery_worker.sync_with_main_site',
        'schedule': crontab(minute='*/10'),  # Каждые 10 минут
    },
    # Очистка временных файлов каждый день в 3 ночи
    'cleanup-temp-files': {
        'task': 'modules.system.python.celery_worker.cleanup_temp_files',
...
def analyze_city_task(self, city: str, user_id: int = None):
    ...
    try:
        from modules.main.python.analysis import process_city
        from modules.main.python.core import state
...
def cleanup_online_users():
    ...
    try:
        from modules.profile.python.activity import get_online_users
...
def analyze_screenshot_task(self, url: str, user_id: int = None):
    ...
    try:
        from modules.main.python.analysis import take_screenshots, analyze_site
        from modules.profile.python.activity import track_user_activity
